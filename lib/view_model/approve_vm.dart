...
class approve_vm extends ChangeNotifier {
  ...
  void removeApproveClient(String idApproveClient) {
    int index = listapprove
        .indexWhere((element) => element.idApproveClient == idApproveClient);
  ...
  Future<void> getApprovebyregoin() async {
    ...
    data = await Api().get(
        url: EndPoints.baseUrls.url +
            'client/approveClientGet.php?fk_regoin=${usercurrent!.fkRegoin}');
  ...
  Future<void> getApprovebycountry() async {
    ...
    data = await Api().get(
        url: EndPoints.baseUrls.url +
            'client/approveClientGet.php?fk_country=${usercurrent!.fkCountry}');
    List<ApproveModel> list = [];
