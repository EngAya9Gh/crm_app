...
class lastcommentclient_vm extends ChangeNotifier {
  ...
  Future<void> getLastcommentClientModel() async {
    ...
    final list =
        List<LastcommentClientModel>.from((response ?? []).map((x) => LastcommentClientModel.fromJson(x)))..sort(
      (a, b) => (num.tryParse(b.hoursLastComment ?? '0') ?? 0).compareTo(int.tryParse(a.hoursLastComment ?? '0') ?? 0),
    );
