...
import '../../../../features/sales/packages_offers/presentation/pages/add_new_package_offer_page.dart';
...
abstract class SalesRoutes {
  static GoRoute allRoutes() {
    return GoRoute(
      ...
      routes: [
        ...
        GoRoute(
          name: AppRoutesPaths.salesSections.packagesOffer.split('/').last,
          path: AppRoutesPaths.salesSections.packagesOffer,
          builder: (context, state) => PackagesOffersPage(),
          routes: _packagesOffersSubSections()
        ),
      ],
    );
  }
  ...
  static List<RouteBase> _clientsSubSections() {
    return [
      GoRoute(
        name: AppRoutesPaths.salesClientsSubSections.clientsList.split('/').last,
      ...
      GoRoute(
        name: AppRoutesPaths.salesClientsSubSections.latestClientsUpdates.split('/').last,
        path: AppRoutesPaths.salesClientsSubSections.latestClientsUpdates,
        builder: (context, state) => LatestClientsUpdatesPage(),
        routes: [
          SharedRoutes.clientProfileRoute(AppRoutesNames.clientProfile.inLatestClientsUpdates),
        ],
      ),
      GoRoute(
        name: AppRoutesPaths.salesClientsSubSections.clientsDebts.split('/').last,
      ...
      GoRoute(
        ...
        routes: [
          SharedRoutes.clientProfileRoute(AppRoutesNames.clientProfile.inClientsTransferApprovals),
        ],
      ),
      GoRoute(
          name: AppRoutesPaths.salesClientsSubSections.pendingInvoices.split('/').last,
          path: AppRoutesPaths.salesClientsSubSections.pendingInvoices,
          builder: (context, state) => PendingInvoicesPage(),
          routes: [
            SharedRoutes.clientDashboardRoute(AppRoutesNames.clientDashboard.inPendingInvoices),
          ]),
      GoRoute(
        name: AppRoutesPaths.salesClientsSubSections.financePending.split('/').last,
        path: AppRoutesPaths.salesClientsSubSections.financePending,
        builder: (context, state) => FinancePendingPage(),
        routes: [
          SharedRoutes.clientDashboardRoute(AppRoutesNames.clientDashboard.inFinancePending),
        ],
      ),
      GoRoute(
        name: AppRoutesPaths.salesClientsSubSections.exceededClients.split('/').last,
        path: AppRoutesPaths.salesClientsSubSections.exceededClients,
        builder: (context, state) => ExceededClientsPage(),
        routes: [
          SharedRoutes.clientProfileRoute(AppRoutesNames.clientProfile.inExceededClients),
        ],
      ),
      GoRoute(
          name: AppRoutesPaths.salesClientsSubSections.clientsContacts.split('/').last,
  ...
  static List<RouteBase> _invoiceSubSections() {
    return [
      GoRoute(
        name: AppRoutesPaths.salesInvoiceSubSections.clientsInvoices.split('/').last,
        path: AppRoutesPaths.salesInvoiceSubSections.clientsInvoices,
        builder: (context, state) => ClientsInvoicesPage(),
      ),
      GoRoute(
        name: AppRoutesPaths.salesInvoiceSubSections.deletedInvoices.split('/').last,
      ...
      GoRoute(
        name: AppRoutesPaths.salesInvoiceSubSections.manageWithdrawnInvoices.split('/').last,
        path: AppRoutesPaths.salesInvoiceSubSections.manageWithdrawnInvoices,
        builder: (context, state) => ManageWithdrawnInvoicesPage(),
      ),
      GoRoute(
        name: AppRoutesPaths.salesInvoiceSubSections.withdrawnInvoices.split('/').last,
  ...
  static List<RouteBase> _relationsSubSections() {
    return [
      GoRoute(
        name: AppRoutesPaths.salesRelationSubSections.participateList.split('/').last,
        path: AppRoutesPaths.salesRelationSubSections.participateList,
        builder: (context, state) => ParticipateListPage(),
        routes: [
          SharedRoutes.participateProfileRoute(AppRoutesNames.participateProfile.inParticipateList),
        ],
      ),
      GoRoute(
        name: AppRoutesPaths.salesRelationSubSections.agentsAndDistributors.split('/').last,
      ...
      GoRoute(
        name: AppRoutesPaths.salesRelationSubSections.companyView.split('/').last,
        path: AppRoutesPaths.salesRelationSubSections.companyView,
        builder: (context, state) => company_view(type: 'ticket'),
      ),
      GoRoute(
        name: AppRoutesPaths.salesRelationSubSections.importantLinks.split('/').last,
        path: AppRoutesPaths.salesRelationSubSections.importantLinks,
        builder: (context, state) => ImportantLinksPage(),
      ),
      GoRoute(
        name: AppRoutesPaths.salesRelationSubSections.usertestView.split('/').last,
  ...
  static List<RouteBase> _raceSubSections() {
    return [
      ...
      GoRoute(
        name: AppRoutesPaths.salesRaceSubSections.salesBranchRace.split('/').last,
        path: AppRoutesPaths.salesRaceSubSections.salesBranchRace,
        builder: (context, state) => BranchRaceView(),
      ),
    ];
  }
  static List<RouteBase> _packagesOffersSubSections() {
    return [
      GoRoute(
        name: AppRoutesPaths.salesPackagesOffersSubSections.addNewItem.split('/').last,
        path: AppRoutesPaths.salesPackagesOffersSubSections.addNewItem,
        builder: (context, state) => AddPackageOfferItem(),
      ),
    ];
  }
}
