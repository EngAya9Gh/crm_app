...
class ReScheduleDialog extends StatefulWidget {
 ...
 final Event event ;
  ReScheduleDialog({Key? key,required this.event,required this.idClientsDate,required this.idinvoice, required this.idClient}) : super(key: key);
  ...
  late String? selectInstallationType ;
    ...
    if (pickedDate != null) //&& pickedDate != currentDate)
      setState(() {
        ...
        final time =Duration(hours: DateTime.now().hour, minutes: DateTime.now().minute, seconds: DateTime.now().second);
                      ...
                      Consumer<invoice_vm>(builder: (context, val, _) {
                         if (val.isloadingRescheduleOrCancel)
                          ...
                          return CustomButton(
                            text: "حفظ",
                            onTap: () async {
                              if(valueInstallationType==null||valueInstallationType!.isEmpty){
                                ...
                                Provider.of<invoice_vm>(context, listen: false)
                                  ...
                                  onSuccess: (value) {
                                    DateTime temp = datetask.hour >= 21 ? datetask.subtract(Duration(hours: 3)) : datetask;
                                      _eventProvider.editEvent(widget.event.copyWith(  
                                          isDone: "3",
                                          // from: temp,
                                  ...
                                  //  Navigator.of(context, rootNavigator: true).pop(false);
                                   Navigator.pop(context, true);
                                    ScaffoldMessenger.of(context).showSnackBar(SnackBar( content: Text("تمت عملية إعادة الجدولة الزيارة  ",
                                        textDirection: myui.TextDirection.rtl )) );
                                  clear();
