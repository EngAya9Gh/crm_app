...
class AppManagerCubit extends Cubit<AppManagerState> {
  ...
  checkAppUpdate() async {
    ...
    response.fold(
          (exception, message) => emit(state.copyWith(updateState: PageState.error())),
          (value) => emit(state.copyWith(updateState: PageState.loaded(data: value.data ?? []))),
    );
  }

  // static Future<Tuple2<bool, bool>?> checkUpdate(List<UpdateConfig> versions) async {
  //   try {
  //     final result = versions.firstWhereOrNull((element) => element.typeVersion == Platform.operatingSystem);
  //
  //     if (result == null) {
  //       return null;
  //     }
  ...
  //
  //     final remoteAppVersion = result.version;
  //
  //     final remoteAppBuildNumber = result.version
  //         .split('+')
  //         .length > 1
  //         ? int.parse(result.version
  //         .split('+')
  //         .last)
  //         : 1;
  //
  //     final updateMandatory = result.mandatory;
  //
