...
class CommissionForCollaboratorsDatasource {
  ...
  Future<List<ParticipateModel>> getParticipate() async {
    try {
      api.changeBaseUrl(EndPoints.baseUrls.urlLaravel);
      final response = await api.get(
        endPoint: EndPoints.finance.participatesSelect,
      );

      return List.of(response['message']).map((e) => ParticipateModel.fromJson(e)).toList();
    } catch (e) {
      debugPrint("error in client dept data source => $e");
      throw Exception("$e");
    }
