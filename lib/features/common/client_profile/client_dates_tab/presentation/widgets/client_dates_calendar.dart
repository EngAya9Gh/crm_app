...
class ClientDatesCalendar extends StatefulWidget {
  const ClientDatesCalendar({super.key});

  @override
  State<ClientDatesCalendar> createState() => _ClientDatesCalendarState();
}

class _ClientDatesCalendarState extends State<ClientDatesCalendar> {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Directionality(
        ...
        child: BlocBuilder<ClientsDatesCubit, ClientsDatesState>(
            ...
            return ConstrainedBox(
              ...
              child: RepaintBoundary(
                child: ModalProgressHUD(
                  inAsyncCall: _cubit.pageVariables.hasReachedEnd?false:_isLoading(state),
                  progressIndicator: AppLoader(),
                  child: TableCalendar<ClientDateModel>(
                    key: UniqueKey(),
  ...
  bool _listenAndBuildWhen(ClientsDatesState previous, ClientsDatesState current) {
    return previous.renderEventsStatus != current.renderEventsStatus ||
        (current.getAllClientsDatesStatus.isLoading()) ||
        _isRefreshUi(previous, current);
  }

  bool _isRefreshUi(ClientsDatesState previous, ClientsDatesState current) =>
      previous.refreshUi != current.refreshUi;

  bool _isLoading(ClientsDatesState state) {
    return (state.getAllClientsDatesStatus.isLoading() );
  }
}

class CalendarFunctions {
  final ClientsDatesCubit datesTableCubit;
  ...
  /* events */
  List<ClientDateModel> eventLoader<ClientDateModel>(day) {
    return (datesTableCubit.pageVariables.eventDataSource[day] ?? [])
        as List<ClientDateModel>;
  }

  /* page */

  /* calendar builders */
  CalendarBuilders<ClientDateModel> calendarBuilders() {
    return CalendarBuilders<ClientDateModel>(
