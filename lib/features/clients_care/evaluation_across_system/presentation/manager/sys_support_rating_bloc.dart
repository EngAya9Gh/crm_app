...
class SysSupportRatingBloc extends Bloc<SysSupportRatingEvent, SysSupportRatingState> {
  ...
  FutureOr<void> _onGetSystemRatingTicketsEvent(GetSystemRatingTicketsEvent event, Emitter<SysSupportRatingState> emit) async {
    ...
    result.extract(
      ...
      (value) => emit(
        state.copyWith(ratingSystemTickets: (value.message?.isEmpty ?? true) ? BlocStatus.empty() : BlocStatus.success(data: value.message)),
      ),
    );
  }

  FutureOr<void> _onAddSystemRatingTicketEvent(AddSystemRatingTicketEvent event, Emitter<SysSupportRatingState> emit) async {
    ...
    result.extract(
      ...
      (value) => emit(
        state.copyWith(
            ...
            ratingSystemTickets: BlocStatus.success(data: List.of(state.ratingSystemTickets.data??[])..add(value.message!))),
      ),
    );
  }
}
